...
from views import list_haulers, retrieve_hauler, delete_hauler, update_hauler, post_hauler
...
class JSONServer(HandleRequests):
    ...
    def do_GET(self):
        ...
        if url["requested_resource"] == "docks":
            ...
            response_body = list_haulers(url['query_params'])
            return self.response(response_body, status.HTTP_200_SUCCESS.value)

        elif url["requested_resource"] == "ships":
            if url["pk"] != 0:
                response_body = retrieve_ship(url["pk"], url['query_params'])
                return self.response(response_body, status.HTTP_200_SUCCESS.value)

            response_body = list_ships(url['query_params'])
            return self.response(response_body, status.HTTP_200_SUCCESS.value)
    ...
    def do_POST(self):
        #url is only needed for post 
        url = self.parse_url(self.path)
        #Python method added by instructor
        content_len = int(self.headers.get('content-length', 0))
        request_body = self.rfile.read(content_len)
        request_body = json.loads(request_body)
        #conditionals for returning success or not
        if url["requested_resource"] == "ships":
